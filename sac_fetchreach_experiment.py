...
"""
train_fetch_sac.py
Stable SAC training for FetchReach-v4 with collapse prevention
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train SAC on FetchReach‑v4 with SB3")
    parser.add_argument("--config", type=str, default="configs/sac_fetchreach_config.yaml", help="Path to config YAML file")
...
def validate_config(cfg: Dict[str, Any]) -> None:
    ...
    if batch_size > buffer_size:
        logging.warning("Batch size larger than buffer size -> invalid config")

    if gradient_steps > 10_000:
        logging.warning("Unusually large gradient_steps for SAC")
...
def train(cfg: Dict[str, Any]) -> None:
    ...
    if train_cfg.get("resume", False):
        ...
        # Log hyperparameters
        logging.info("SAC Hyperparameters:")
        ...
        manual_keys = {
            "policy", "verbose", "policy_kwargs",
            "replay_buffer_class", "replay_buffer_kwargs"
        }
        auto_kwargs = {
            k: v for k, v in algo_cfg.items()
            if k not in manual_keys
        }
        model = SAC(
            algo_cfg["policy"],
            env,
            replay_buffer_class=HerReplayBuffer if algo_cfg.get("replay_buffer_class") else None,
            replay_buffer_kwargs=algo_cfg.get("replay_buffer_kwargs", {}),
    ...
    raw_eval_freq = cfg["evaluation"].get("eval_freq_timesteps", 10000)
    ...
    if cfg.get("evaluation", {}).get("enabled", False):
        ...
        raw_eval_freq = eval_cfg.get("eval_freq_timesteps", 10000)
    ...
    logging.info(f"train_freq: {model.train_freq}")
    logging.info(f"gradient_steps: {model.gradient_steps}")
    logging.info(f"batch_size: {model.batch_size}")    
    logging.info(f"buffer_size: {model.buffer_size}")
...
def main():
    ...
    logging.info("Starting SAC training on FetchReach-v4")
